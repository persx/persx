...
// Content without the duplicate TL;DR at the top
const contentWithoutTldr = `## Highlights by Vendor
...
async function removeDuplicateTldr() {
  const slug = 'ai-for-experimentation-emerging-trends';

  console.log('📝 Removing duplicate TL;DR from content markdown...\n');

  const { data, error } = await supabase
    .from('knowledge_base_content')
    .update({
      content: contentWithoutTldr,
  ...
  console.log('✅ Duplicate TL;DR removed!\n');
  console.log('Now the article shows:');
  console.log('─'.repeat(80));
  console.log('1. TL;DR in blue box at top (from overall_summary field)');
  console.log('2. Content starts directly with "## Highlights by Vendor"');
  console.log('─'.repeat(80));
  console.log('\n🌐 View at: http://localhost:3000/news/' + slug);
}

removeDuplicateTldr().catch(console.error);
